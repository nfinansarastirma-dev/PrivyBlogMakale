  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" data-testid="dashboard">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b border-white/10 pb-8">
        <div>
          <div className="font-jetbrains text-[10px] uppercase tracking-widest text-[#10B981]">// {isAdmin ? "admin.console" : "writer.console"}</div>
          ...
          <div className="mt-3 flex items-center gap-3">
            {user.picture ? (
              ...
              <div className="w-9 h-9 bg-[#10B981] text-black flex items-center justify-center font-jetbrains font-bold">{user.name?.[0]?.toUpperCase()}</div>
            )}
            <div>
              ...
              <p className="font-jetbrains text-[10px] uppercase tracking-widest text-white/50">
                {isAdmin ? <span className="text-[#F59E0B]">admin</span> : "writer"} · {user.email}
              </p>
            </div>
          </div>
        </div>
        <Link
          ...
          className="inline-flex items-center gap-2 bg-[#10B981] hover:bg-[#0EA371] text-black px-5 py-3 font-jetbrains text-[11px] uppercase tracking-widest"
        >
...
const TabBtn = ({ active, onClick, children, icon, tid }) => (
  <button
    ...
    className={`inline-flex items-center gap-2 px-4 py-3 font-jetbrains text-[11px] uppercase tracking-widest border-b-2 -mb-px transition-colors ${
      active ? "border-[#10B981] text-[#10B981]" : "border-transparent text-white/50 hover:text-white"
    }`}
  >
    {icon}{children}
  </button>
          ...
          {articles.map(a => (
            <div key={a.id} data-testid={`admin-row-${a.id}`} className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 px-4 py-4 border-b border-white/5 last:border-b-0 items-center hover:bg-white/[0.02]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-jetbrains text-[10px] uppercase tracking-widest text-white/40">
                      ...
                      {a.featured && <span className="ml-2 text-[#F59E0B]">★ öne çıkan</span>}
                    </p>
                  </div>
                </div>
              </div>
              <div className="col-span-2 font-jetbrains text-[11px] text-[#10B981]">{a.category_name}</div>
              <div className="col-span-1">
                <span className={`inline-block px-2 py-0.5 font-jetbrains text-[10px] uppercase tracking-widest ${a.status === "published" ? "bg-[#10B981] text-black" : "border border-white/20 text-white/70"}`}>
              ...
              <div className="col-span-2 flex md:justify-end gap-2 flex-wrap">
                <Link to={`/dashboard/edit/${a.id}`} data-testid={`edit-${a.id}`} className="p-2 border border-white/10 hover:border-[#10B981] text-white/70 hover:text-[#10B981]"><Edit3 size={14} /></Link>
                <button data-testid={`publish-${a.id}`} onClick={() => publish(a)} className="p-2 border border-white/10 hover:border-[#F59E0B] text-white/70 hover:text-[#F59E0B]" title={a.status === "published" ? "Taslağa al" : "Yayınla"}>
                  <FileText size={14} />
                </button>
                {isAdmin && (
                  <button data-testid={`feature-${a.id}`} onClick={() => feature(a)} className={`p-2 border border-white/10 hover:border-[#F59E0B] ${a.featured ? "text-[#F59E0B]" : "text-white/70 hover:text-[#F59E0B]"}`} title="Öne çıkar">
                    <Star size={14} />
                  </button>
                )}
      ...
      {users.map(u => (
        <div key={u.user_id} data-testid={`user-row-${u.user_id}`} className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 px-4 py-4 border-b border-white/5 last:border-b-0 items-center">
          <div className="col-span-4 flex items-center gap-3">
            {u.picture ? <img src={u.picture} alt="" className="w-9 h-9 border border-white/10" /> : <div className="w-9 h-9 bg-[#10B981] text-black flex items-center justify-center font-jetbrains font-bold">{u.name?.[0]?.toUpperCase()}</div>}
          ...
          <div className="col-span-2">
            <span className={`inline-block px-2 py-0.5 font-jetbrains text-[10px] uppercase tracking-widest ${u.role === "admin" ? "bg-[#F59E0B] text-black" : "border border-white/20 text-white/70"}`}>
              {u.role}
            </span>
          </div>
          <div className="col-span-2 flex md:justify-end gap-2">
            {u.role === "writer" ? (
              <button data-testid={`promote-${u.user_id}`} onClick={() => changeRole(u, "admin")} className="px-3 py-1.5 border border-white/10 hover:border-[#F59E0B] text-white/70 hover:text-[#F59E0B] font-jetbrains text-[10px] uppercase tracking-widest inline-flex items-center gap-1">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <form onSubmit={create} className="md:col-span-1 border border-white/10 p-5 space-y-3" data-testid="new-category-form">
        ...
        <input data-testid="new-cat-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Kategori adı" className="w-full bg-black border border-white/10 px-3 py-2 outline-none focus:border-[#10B981] font-jetbrains text-sm" />
        <textarea data-testid="new-cat-desc" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Açıklama" rows={3} className="w-full bg-black border border-white/10 px-3 py-2 outline-none focus:border-[#10B981] font-jetbrains text-sm" />
        <Button type="submit" data-testid="new-cat-submit" className="w-full bg-[#10B981] hover:bg-[#0EA371] text-black rounded-none font-jetbrains text-[11px] uppercase tracking-widest">Ekle</Button>
      </form>
